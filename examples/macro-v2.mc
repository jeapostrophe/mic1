...
pc := band(amask, ir); wr; goto START; 
...
TEST_1111_11?:
tir := lshift(tir); if n then goto TEST_1111_111?;

TEST_1111_110?:
tir := lshift(tir); if n then goto TEST_1111_1101?;
...
TEST_1111_1101?:
tir := lshift(tir); if n then goto TEST_1111_1101_1?;

TEST_1111_1101_0?:
tir := lshift(tir); if n then goto MULC;

LSHIFT: {IR = 1111 1101 00?? ????}
ac := lshift(ac); goto START;

MULC: {IR = 1111 1101 01?? ????}
a := lshift(1);     { a = 00 0010 }
a := lshift(a + 1); { a = 00 0110 }
a := lshift(a + 1); { a = 00 1110 }
a := lshift(a + 1); { a = 01 1110 }
a := lshift(a + 1); { a = 11 1110 }
a := a + 1;         { a = 11 1111 }
b := band(ir, a);   { b = bottom 6-bits }
c := 0;
MULC_loop:
b := b + (-1); if n then goto MULC_end;
c := ac + c; goto MULC_loop;
MULC_end:
ac := c; goto START;

TEST_1111_1101_1?:
tir := lshift(tir); if n then goto ANDC;

BNEG: {IR = 1111 1101 10?? ????}
ac := inv(ac); goto START;

ANDC: {IR = 1111 1101 11?? ????}
mar := pc; rd;
pc := pc + 1; rd;
ac := band(mbr, ac); goto START;

TEST_1111_111?:
tir := lshift(tir); if n then goto HALT; 
